...
pkg="base-devel mesa arch-install-scripts usbutils bash-language-server xorg util-linux-libs gptfdisk iputils net-tools nftables perl wireless_tools iw busybox dracut systemd-ui systemd-sysvcompat f2fs-tools xfsprogs attr linux-firmware kmod cronie bc discover flatpak fwupd partitionmanager gstreamer phonon-qt5 gstreamer gstreamer-vaapi qt-gstreamer python-pip openssh putty rng-tools rt-tests shellcheck uget ccache vlc hdparm ethtool xsettingsd virt-manager nmap apparmor diffuse htop psensor wireless-regdb binwalk rtirq lz4 shellcheck kvantum-theme-materia kvantum kio-fuse kio-extras kio firewalld kwrite dolphin dolphin-plugins irqbalance openssl efibootmgr dbus-broker xorg-xinit fail2ban android-tools android-udev dpkg lz4 macchanger kdebugsettings ffmpeg ffmpeg4.4 x264 x265 aom dav1d svt-av1 schroedinger libdv libmpeg2 libtheora libvpx libva-mesa-driver mesa-utils mesa-vdpau opencl-mesa vulkan-mesa-layers libva-mesa-driver libva-utils libva-vdpau-driver libvdpau-va-gl vdpauinfo vulkan-icd-loader spectacle openbox imagemagick vim powerline-vim x86_energy_perf_policy openh264 glxinfo sweeper xz zstd pigz pbzip2 gst-libav upower libcdio libdvdread libsidplay gst-plugins-base gst-plugins-good gst-plugins-ugly gst-plugins-bad libdvdcss os-prober lsb-release reflector tcc perl-locale-gettext sxiv zathura ranger nix i7z"
